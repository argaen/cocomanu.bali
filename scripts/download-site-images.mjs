...
const IMAGES_DB_ID = process.env.NOTION_IMAGES_DB_ID || '396452d3e0138000a918e92ffb1b3256';
const OUTPUT_DIR = path.resolve(process.cwd(), 'public/images/site');
...
async function fetchAllImagePages() {
  const pages = [];
  let cursor;

  while (true) {
    const response = await notion.databases.query({
      database_id: IMAGES_DB_ID,
      start_cursor: cursor,
      page_size: 100,
    });
...
async function removeOrphanSiteImages(validSlugs) {
  let removed = 0;
  let entries;
  try {
...
async function main() {
  ...
  const pages = await fetchAllImagePages();
  ...
  const removed = await removeOrphanSiteImages(validSlugs);
  if (removed > 0) {
  ...
  for (const page of pages) {
    ...
    if (!imageUrl) {
      skippedEntries.push({ label: slug, reason: 'missing image' });
      continue;
    }
